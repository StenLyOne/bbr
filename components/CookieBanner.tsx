...
import { getConsent, setConsent } from "../lib/cookiesConsent";
...
export default function CookieBanner() {
  ...
  useEffect(() => {
    ...
    if (!consent) {
      setVisible(true); // первый визит
    } else {
      setShowReopen(true); // уже есть решение → показываем кнопку
    }
  }, []);
      ...
      {visible && (
        <div className="fixed bottom-4 left-4 z-[9999] bg-blue text-white px-4 py-4 w-[80%] md:max-w-[360px] rounded-2xl">
          <p className="text-sm leading-[100%]">
            ...
            <Link className="underline text-blue-500" href="/cookies">
              cookies
            </Link>
          </p>
          ...
          <div className="mt-5 flex gap-2">
            <button
              onClick={acceptAll}
              className="bg-white text-black px-3 py-1 text-[14px]  font-semibold  rounded-2xl cursor-pointer hover:opacity-80"
            >
              Accept
            </button>
            ...
            <button
              onClick={rejectAll}
              className="border border-white/40 px-3 py-1 text-[14px]  rounded-2xl  cursor-pointer hover:opacity-80"
            >
              Reject
            </button>
          </div>
